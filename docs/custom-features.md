...
## 3. Station ID Column in Channels Tab

### What it does
The Channels tab table shows a **Station ID** column between Name and Source. It displays the Gracenote station ID for each channel at a glance, making it easy to verify guide data assignments without opening the edit form.

### Why upstream doesn't have it
Upstream does not expose station IDs in the channel list view.

### Files changed
- [src/routes/config.ts](../src/routes/config.ts) — table header + row cell + edit row colspan
- [src/routes/root.ts](../src/routes/root.ts) — column width + responsive CSS + provider UI cell index

### Validation
- Open the web UI → Channels tab.
- Confirm a **Station ID** column appears between Name and Source.
- Channels with a station ID (e.g. `ae`, `amc`) show their numeric ID; channels without one (e.g. `abc`) show an empty cell.
- On a viewport ≤ 1024 px wide the Station ID column is hidden alongside Profile.
- Enable a previously-disabled predefined channel — the Source column must show the provider dropdown/name, not `auto`.
- Open the Edit form for a user channel — the form must span all 6 columns without a gap.

### Implementation

#### A. Row cell — `src/routes/config.ts` (`generateChannelRowHtml`)
...
Insert the new cell after the name cell:
```typescript
  displayLines.push("<td>" + escapeHtml(channel.name ?? key) + "</td>");
...
#### B. Table header — `src/routes/config.ts` (`generateChannelsPanel`)
...
#### C. Column width — `src/routes/root.ts` (`generateLandingPageStyles`)
...
#### D. Responsive CSS — `src/routes/root.ts` (`generateLandingPageStyles`)
...
#### E. Provider UI cell index — `src/routes/root.ts` (`updateChannelProviderUI`)

The column insertion shifts the Profile cell from index 3 to index 4. Without this fix, changing a provider (or bulk-assigning providers) writes `auto` into the Source column instead of Profile.
...
Replace with:
```typescript
    "    var profileCell = row.cells[4];",
```

#### F. Edit row colspan — `src/routes/config.ts` (`generateChannelRowHtml`)

The edit form row spans all table columns via `colspan`. Adding the Station ID column increases the count from 5 to 6.
...
## 4. Loading Slate — Stream Immediately with Logo While Channel Tunes (PLANNED, NOT IMPLEMENTED)
